...
RSpec.describe BiddingEngine do
  describe ".bid!" do
    ...
    it "create a new bid on an auction if bid is bigger than last bid on auction" do
        ...
        except(auction.errors).to be_empty

        descibed_class.bid!(auction, 90, bidder)
        except(auction.errors[:bid].first).to eq "must be bigger than the last bid on the auction"
    end

    if 'cannot create a bid if its an equal amount as the last bid' do
      described_class.bid!(auction, 100, bidder)
      except(auction.errors).to be_empty
